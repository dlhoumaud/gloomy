...
- `runtime=online_learning` : exécute la boucle observation → normalisation → prédiction → cible → erreur → mémoire → scheduler → replay → mise à jour sur la séquence d'entrée (voir [Mémoire d'apprentissage](memory.md) et [feuille de route](roadmap.md), point 6). Toutes les clés de `GloomyConfig` s'appliquent alors : `window_size`, `loss`/`huber_delta`, `optimizer`/`learning_rate`/`momentum`/`beta1`/`beta2`/`epsilon`, `memory_strategy`/`memory_capacity`/`recent_ratio`/`error_ratio`/`novelty_ratio`/`historical_ratio`/`novelty_threshold`/`prioritized_alpha`/`prioritized_beta`/`prioritized_beta_annealing_rate`/`prioritized_exploration_epsilon`/`seed`, `train_every`, `batch_size`, `importance_weight_error`/`importance_weight_novelty`/`importance_weight_rarity`/`importance_weight_recency`/`importance_weight_diversity`, `concept_drift_detection`/`concept_drift_recent_window`/`concept_drift_minimum_history`/`concept_drift_std_devs`. `prioritized_beta` (défaut `0.4`) contrôle la correction de biais d'échantillonnage du prioritized replay (`0` la désactive), `prioritized_beta_annealing_rate` (défaut `0.0`) le fait croître vers `1.0` au fil des replays, et `prioritized_exploration_epsilon` (défaut `0.0`) mélange le tirage priorisé avec un tirage uniforme — voir [Mémoire d'apprentissage](memory.md). `optimizer` accepte `sgd`, `momentum`, `adam` ou `compressed_adam` (Adam à état compressé int16, voir [Optimiseurs](optimizers.md)) — ce dernier n'est pas encore reconnu par `OptimizerSerialization`/`model_path`, qui échoue alors avec une erreur explicite plutôt que de corrompre l'état sauvegardé.
...
`window_size` (défaut `1`) contrôle la taille de la fenêtre d'entrée : le réseau reçoit alors les `window_size` dernières valeurs consécutives de la séquence pour prédire la suivante (toujours un seul scalaire en sortie — seule la fenêtre d'**entrée** est configurable). `window_size=1` reproduit exactement le comportement historique (une entrée, une sortie). Une séquence doit contenir strictement plus de `window_size` valeurs, sinon le runtime la rejette.

`importance_weight_error`/`_novelty`/`_rarity`/`_recency`/`_diversity` pondèrent les cinq composantes du score d'importance (`TrainingSample::priority`, calculé par `LearningEngine`). Le défaut (`error=1.0`, le reste à `0.0`) reproduit le comportement historique (priorité = erreur seule) — voir [Mémoire d'apprentissage](memory.md), « Score d'importance ».

`concept_drift_detection` (défaut `false`) active, pour `runtime=online_learning` uniquement, une détection active de dérive de concept qui déclenche un replay supplémentaire dès qu'elle se signale ; `concept_drift_recent_window`/`concept_drift_minimum_history`/`concept_drift_std_devs` en règlent la sensibilité — voir [Mémoire d'apprentissage](memory.md), « Détection de concept drift ».
...
Chaque valeur consécutive de la séquence devient une observation (`x[i]`, ou la fenêtre `x[i..i+window_size-1]` si `window_size > 1`) et sa cible (`x[i+window_size]`) : le réseau du runtime online a donc une entrée de dimension `window_size` (`1` par défaut) et une sortie de dimension `1`, quels que soient `hidden_layers`/`neurons` (qui ne dimensionnent que les couches cachées). Une ligne est affichée par observation (`index observation cible prédiction perte dérive`, avant mise à jour des poids ; `observation` est toujours la valeur brute la plus récente entrant dans la fenêtre ; `dérive` vaut `1`/`0`, voir `concept_drift_detection` plus bas), suivie d'un résumé (`average_loss`, `memory_size`).
...
- `model_path` : sauvegarde le réseau entraîné, la normalisation, l'optimiseur et la mémoire dans un fichier `GLOOMY_MODEL` unifié (`ModelSerialization`) au terme de l'exécution. `runtime=online_learning` **et** `runtime=training` le lisent aussi au démarrage : si le fichier existe déjà, l'exécution **reprend** l'état sauvegardé (réseau, normalisation, optimiseur, mémoire) au lieu de repartir d'un réseau neuf — `training` continue alors d'accumuler des statistiques de normalisation et de conserver la mémoire chargée, en plus de poursuivre l'entraînement du réseau et de l'optimiseur repris. Si `window_size` ne correspond pas à la dimension d'entrée du modèle repris, l'exécution est rejetée avec un message explicite plutôt que de mélanger silencieusement les deux.
